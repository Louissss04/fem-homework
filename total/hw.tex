...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{equation}\label{poincare}
			\|v\|_{m,\Omega}^2\leq C_2\Big(|v|_{m,\Omega}^2+\sum_{|\alpha|<m}\Big(\int_\Omega\partial^\alpha v\mathrm{dx}\Big)^2\Big),\forall v\in H^m(\Omega),
		\end{equation}
	...
	\begin{enumerate}
		...
		\begin{proof}
			...
			$$\begin{aligned}
				...
				& \leq \int_e v^2 ds - 2c \int_e v ds + c^2 |e| \qquad \text{（均值不等式）}\\
				& = \int_e (v-c)^2 ds\\
				& = \|v-c\|_{0,e}^2
			\end{aligned}$$
